...
class ActiveField extends \kartik\form\ActiveField
{
    ...
    protected function create_date($date_value){
        if($date_value){
            Yii::$app->formatter->asFormDateTime(date_create( $date_value));
        }
        return $date_value;
    }
    ...
    public function dateTimeInput(array $options = []): ActiveField
    {
        if (!array_key_exists('value', $options)) {
            $options['value'] = $this->create_date(\yii\helpers\Html::getAttributeValue($this->model, $this->attribute));
        }
